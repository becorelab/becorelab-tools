# Coupang Partners Pipeline - Windows Task Scheduler Setup
# 3 scheduled tasks for automated youtuber outreach
# Updated 2026-04-19: Added StartWhenAvailable, WakeToRun, battery allowances

$pythonExe = "C:\Users\info\ClaudeAITeam\mcp-server\.venv\Scripts\python.exe"
$pipelineScript = "C:\Users\info\claudeaiteam\marketing\coupang_partners\pipeline.py"
$workDir = "C:\Users\info\claudeaiteam\marketing\coupang_partners"

# Common settings (applied to all tasks)
$settings = New-ScheduledTaskSettingsSet `
    -StartWhenAvailable `
    -WakeToRun `
    -AllowStartIfOnBatteries `
    -DontStopIfGoingOnBatteries `
    -ExecutionTimeLimit (New-TimeSpan -Hours 1) `
    -MultipleInstances IgnoreNew

# 1) Daily Crawl - Weekday 09:00
$action1 = New-ScheduledTaskAction -Execute $pythonExe -Argument "$pipelineScript crawl" -WorkingDirectory $workDir
$trigger1 = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Monday,Tuesday,Wednesday,Thursday,Friday -At 9am
Register-ScheduledTask -TaskName "CoupangPartners_Crawl" -Action $action1 -Trigger $trigger1 -Settings $settings -Description "Daily YouTube crawl + Haiku screening" -Force

# 2) Daily Send - Weekday 10:00
$action2 = New-ScheduledTaskAction -Execute $pythonExe -Argument "$pipelineScript send" -WorkingDirectory $workDir
$trigger2 = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Monday,Tuesday,Wednesday,Thursday,Friday -At 10am
Register-ScheduledTask -TaskName "CoupangPartners_Send" -Action $action2 -Trigger $trigger2 -Settings $settings -Description "Send outreach emails to approved youtubers" -Force

# 3) Daily Reply Check - Weekday 14:00 and 18:00
$action3 = New-ScheduledTaskAction -Execute $pythonExe -Argument "$pipelineScript check" -WorkingDirectory $workDir
$trigger3a = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Monday,Tuesday,Wednesday,Thursday,Friday -At 2pm
$trigger3b = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Monday,Tuesday,Wednesday,Thursday,Friday -At 6pm
Register-ScheduledTask -TaskName "CoupangPartners_Check" -Action $action3 -Trigger @($trigger3a, $trigger3b) -Settings $settings -Description "Check for youtuber email replies" -Force

Write-Host ""
Write-Host "Scheduled tasks registered with improved settings:"
Write-Host "  CoupangPartners_Crawl  - Weekday 09:00"
Write-Host "  CoupangPartners_Send   - Weekday 10:00"
Write-Host "  CoupangPartners_Check  - Weekday 14:00, 18:00"
Write-Host ""
Write-Host "Applied settings:"
Write-Host "  StartWhenAvailable             : catch up missed runs"
Write-Host "  WakeToRun                      : wake PC from sleep"
Write-Host "  AllowStartIfOnBatteries        : run even on battery"
Write-Host "  DontStopIfGoingOnBatteries     : keep running on battery switch"
