...
def explode_products(name, opt, q):
    ...
    if "단독 공구" in name:
    ...
    else:  # 단품 (구체적인 조건부터 — 옵션에 묶음 수량이 명시된 경우 우선)
        # 샘플류 (무료키트 + 유료 테스트샘플 6정)
        if "샘플 키트" in name or "테스트샘플" in name or "맛보기용 샘플" in opt:
            add("식세기 샘플키트", 1)
        ...
        # 캡슐 세탁세제 초고농축 — 옵션의 "캡슐세제 N개" 수량 파싱
            ...
            add("캡슐표백제", 1)
        elif "얼룩제거제 350" in name or "얼룩제거제 100" in name:
            add("얼룩제거제", 1)
        elif "섬유탈취제 400" in name or "섬유탈취제 100" in name:
            add("섬유탈취제", 1)
        # 퍼퓸 건조기시트 단품 "○○ 향기 40매" — 옵션의 "N개입" 수량 파싱 (향은 상품명 기준)
        elif "향기" in name and "40매" in name:
            m = re.search(r"(\d+)개입", opt)
...
def write_xlsx(day, hour, sku, prod_total, tot):
    """구글드라이브에 통합 엑셀 1개(시트 5개) 저장 — 윈도우 PC 열람용."""
    try:
...
def report(master):
    ...
    xlsx = write_xlsx(day, hour, sku, prod_total, tot)
