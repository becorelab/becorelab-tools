...
# 6월 채널 총계 (2026-07-14 마감 확정, build_june_consolidated — 이익=배송수익 포함 대칭 정의)
JUN={'카페24':(24533116,17153123),'스마트스토어':(49467040,31666659),'쿠팡(로켓배송)':(46974511,24885484),
'쿠팡(로켓그로스)':(24845425,8605277),'지마켓':(1586186,1053397),'옥션':(425275,269497),'11번가':(1351612,834519),
'SSG':(96600,74583),'GS샵':(30160,20223),'오늘의집':(58380,37430),'에이블리':(45333,29649),
'카카오선물하기':(101760,72021),'카카오쇼핑하기':(113949,88501),'라이플로우':(19578280,4236786),
'PG컴퍼니':(977990,200166),'두버':(1069460,502893),'에드가':(45690,25556)}
# 6월 채널별 VAT별도 이익 (2026-07-14 확정: (매출+배송)/1.1 − 원가(VAT별도) − 그로스물류/1.1. 수동발주 정산액=VAT포함(대표님 확인))
JUN_NET={'카페24':14922840,'스마트스토어':27169655,'쿠팡(로켓배송)':20615074,'쿠팡(로켓그로스)':6867439,
'지마켓':909198,'옥션':230836,'11번가':711645,'SSG':65801,'GS샵':17481,'오늘의집':32123,'에이블리':25528,
'카카오선물하기':62770,'카카오쇼핑하기':78142,'라이플로우':2456942,'PG컴퍼니':111258,'두버':405669,'에드가':21402}
...
# ── VAT별도 이익 시트 (2026-07-14 신설: 공식 순이익 지표) ──
# 1~5월 = 이익/1.1 (매출·원가 모두 VAT포함 차원이라 정확 환산) / 6월 = 채널별 정밀 계산(JUN_NET)
ws_net=wb.create_sheet('채널별 이익(VAT별도)')
ws_net['A1']='2026 채널별 월별 이익 — VAT별도 (부가세 제외 순이익, 공식 지표)'; ws_net['A1'].font=Font(bold=True,size=13)
hdrn=['그룹','채널']+MONTHS+['합계']
ws_net.append([]); ws_net.append(hdrn)
for c in range(1,len(hdrn)+1): ws_net.cell(row=3,column=c).fill=HF; ws_net.cell(row=3,column=c).font=HFONT
def net_val(ch,mn):
    if mn=='6월': return JUN_NET.get(ch,0)
    return M[ch][mn][1]/1.1
r=4; ntot={mn:0 for mn in MONTHS}
for g,chs in GROUPS.items():
    chs=[c for c in dict.fromkeys(chs) if c in M and active(c)]
    if not chs: continue
    gs=r
    for ch in chs:
        vals=[net_val(ch,mn) for mn in MONTHS]
        ws_net.append([g if ch==chs[0] else '',ch]+[round(v) for v in vals]+[round(sum(vals))])
        for c in range(3,len(hdrn)+1): mm(ws_net.cell(row=r,column=c))
        for i,mn in enumerate(MONTHS): ntot[mn]+=vals[i]
        r+=1
    if len(chs)>1: ws_net.merge_cells(start_row=gs,start_column=1,end_row=r-1,end_column=1)
    ws_net.cell(row=gs,column=1).alignment=Alignment(vertical='center',horizontal='center'); ws_net.cell(row=gs,column=1).font=Font(bold=True)
ws_net.append(['','■ 종합']+[round(ntot[mn]) for mn in MONTHS]+[round(sum(ntot.values()))])
for c in range(1,len(hdrn)+1): ws_net.cell(row=r,column=c).fill=TOT; ws_net.cell(row=r,column=c).font=Font(bold=True,size=11)
for c in range(3,len(hdrn)+1): mm(ws_net.cell(row=r,column=c))
ws_net.cell(row=r+2,column=1,value='※ 1~5월=확정이익÷1.1(전항목 VAT포함 차원). 6월=(매출+배송)÷1.1−원가(VAT별도)−그로스물류÷1.1, 수동발주 정산액=VAT포함(대표님 확인 7/14). 광고비·운임 차감 전.')
for col,w in zip('ABCDEFGHIJ',[9,20,12,12,12,12,12,12,12]): ws_net.column_dimensions[col].width=w
